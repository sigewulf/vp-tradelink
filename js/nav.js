// V P Tradelink — shared mobile nav toggle. One file, included identically on every page.
(function () {
  try {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (langSwitch && langTrigger) {
    ...
    // ...and offer it once per session on later visits, using the
    // already-generated cross-language link for this exact page so the
    // visitor lands on the matching page, not just the homepage.
    //
    // Deliberately NOT an auto-redirect: a silent navigation can surprise a
    // visitor who has a good reason to be on a non-preferred-language page
    // (a shared link, a bilingual colleague, testing from another device).
    // Instead we show a small dismissible banner and let them choose.
    try {
      var storedLang = localStorage.getItem("vp_lang");
      var currentLang = document.documentElement.getAttribute("lang");
      var alreadyOffered = sessionStorage.getItem("vp_lang_redirected");
      if (storedLang && storedLang !== currentLang && !alreadyOffered && !window.location.search) {
        ...
        if (targetLink) {
          ...
          var bannerCopy = {
            en: { text: "Continue in English instead?", action: "Switch to English" },
            id: { text: "Lanjutkan dalam Bahasa Indonesia?", action: "Ganti ke Bahasa Indonesia" },
            vi: { text: "Tiếp tục bằng Tiếng Việt?", action: "Chuyển sang Tiếng Việt" }
          }[storedLang];
          ...
          if (bannerCopy) {
            ...
            textSpan.textContent = bannerCopy.text;
            var actionLink = document.createElement("a");
            actionLink.href = targetLink.href;
            actionLink.className = "btn btn--primary lang-banner__action";
            actionLink.textContent = bannerCopy.action;
            var dismissBtn = document.createElement("button");
            dismissBtn.type = "button";
            dismissBtn.className = "lang-banner__dismiss";
  ...
  // Sticky mobile CTA bar: stays off-screen until the visitor has scrolled
  // past the hero quick-facts (Origin, MOQ, Lead Time, etc.), so it never
  // covers that content on initial load. Reappears once scrolled back up.
  var stickyBar = document.querySelector(".btn-sticky-mobile");
  var quickFacts = document.querySelector(".quick-facts");
  if (stickyBar && quickFacts && "IntersectionObserver" in window) {
    var revealObserver = new IntersectionObserver(function (entries) {
      var entry = entries[0];
      stickyBar.classList.toggle("is-visible", !entry.isIntersecting && entry.boundingClientRect.top < 0);
    }, { threshold: 0 });
    revealObserver.observe(quickFacts);
  } else if (stickyBar) {
    // No quick-facts on this page (or no IntersectionObserver support) — keep prior always-visible behaviour.
